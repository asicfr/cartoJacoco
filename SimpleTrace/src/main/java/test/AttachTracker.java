...
public class AttachTracker {
	...
	// Set up a launching connection to the JVM
	{
		...
		AttachingConnector conn = getCommandLineConnector();
		// Map<String,Connector.Argument> connArgs = setMainArgs(classpathStr,
		// conn, args, vmArgs);

		try {
			Map<String, Argument> defaults = conn.defaultArguments();
			...
			vm = conn.attach(defaults); // launch the JVM and connect to it
		} catch (IOException e) {
	...
	private AttachingConnector getCommandLineConnector()
	// find a command line launch connector
	{
		...
		for (Connector conn : conns) {
			if (conn.name().equals("com.sun.jdi.SocketAttach"))
				return (AttachingConnector) conn;
		}
	...
	// make the tracer's input arguments the program's main() arguments
	{
		...
		mArgs.setValue(sb.toString()); // assign input args to application's
										// main()
		return connArgs;
	} // end of setMainArgs()
	...
	// monitor the JVM running the application
	{
		...
		try {
			watcher.join(); // Wait. Shutdown begins when the JDI watcher
							// terminates
			// errRedirect.join(); // make sure all the stream outputs have been
								// forwarded before we exit
			// outRedirect.join();
...
//			}
			new AttachTracker(classpathStr, args, null);
//		}
	}
